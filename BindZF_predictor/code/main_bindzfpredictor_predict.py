...
#import tensorflow.python.keras.optimizer_v2
import tensorflow as tf
import keras
...
def predict_by_len(model_generator, input_encoder, output_spec, seqs, raw_Y, start_seq_len = 512, start_batch_size = 32, increase_factor = 2):
    ...
    assert len(y_trues) > 0, "No data in y_trues array"
    assert len(y_preds) > 0, "No data in y_preds array"
...
def main(args):
    ...
    # model_generator = tf.keras.models.load_model('BindZF_predictor/code/model.h5')
